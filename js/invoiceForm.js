...
InvoiceForm.prototype.formatAmount = function(amount, currency = 'MZN', showBoth = false) {
    ...
    if (currency === 'MZN' || !showBoth) {
        return mznFormatter.format(amount);
    }

    // For foreign currency, show both amounts
    const foreignAmount = amount / (this.currentRate || 1);
    return `${foreignFormatter.format(foreignAmount)} (${mznFormatter.format(amount)})`;
};

// Add exchange rate update method
InvoiceForm.prototype.updateExchangeRate = async function(currency) {
    if (currency === 'MZN') {
        this.currentRate = 1;
        return;
    }
    ...
    try {
        // Use a fallback rate if API is not available
        const rates = {
            'USD': 64.25,
            'EUR': 70.50,
            'ZAR': 3.45
        ...
        this.currentRate = rates[currency] || 1;
        
        // Update the rate info display
        const rateInfo = document.getElementById('exchangeRateInfo') || document.createElement('div');
        rateInfo.id = 'exchangeRateInfo';
        rateInfo.style.marginTop = '10px';
        rateInfo.style.fontSize = '0.9em';
        rateInfo.style.color = '#666';
        rateInfo.textContent = `Taxa de câmbio: 1 ${currency} = ${this.currentRate.toFixed(2)} MZN`;
        
        const totalsSection = document.querySelector('.invoice-totals');
        if (totalsSection && !document.getElementById('exchangeRateInfo')) {
            totalsSection.appendChild(rateInfo);
        }
    } catch (error) {
...
InvoiceForm.prototype.updateItemRow = function(row) {
    ...
    row.querySelector('.item-discounted-subtotal').textContent = this.formatAmount(discountedSubtotal, currency, true);
    row.querySelector('.item-vat').textContent = this.formatAmount(vat, currency, true);
    row.querySelector('.item-total').textContent = this.formatAmount(total, currency, true);
...
InvoiceForm.prototype.updateReviewTotals = function() {
    ...
    document.getElementById('subtotal').textContent = this.formatAmount(subtotal, currency, true);
    document.getElementById('totalDiscount').textContent = this.formatAmount(totalDiscount, currency, true);
    document.getElementById('subtotalAfterDiscount').textContent = this.formatAmount(subtotalAfterDiscount, currency, true);
    document.getElementById('totalVat').textContent = this.formatAmount(totalVat, currency, true);
    document.getElementById('invoiceTotal').textContent = this.formatAmount(grandTotal, currency, true);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Store original values to restore when toggling off
    let originalValues = {
        name: '',
        email: '',
        address: '',
    ...
    // Toggle General Client mode
    if (generalClientBtn) {
        generalClientBtn.addEventListener('click', function() {
            const isGeneralClient = clientInput.dataset.generalClient === 'true';
            
            if (isGeneralClient) {
                // Turning off General Client mode
                generalClientBtn.classList.remove('highlighted');
                clientInput.dataset.generalClient = '';
                clientInput.value = originalValues.name;
                clientEmail.value = originalValues.email;
                clientAddress.value = originalValues.address;
                clientTaxId.value = originalValues.taxId;
                clientFields.forEach(f => f.style.display = '');
                clientInput.removeAttribute('readonly');
            } else {
                // Turning on General Client mode
    ...
    // If user manually edits client input when not in General Client mode
    if (clientInput) {
        clientInput.addEventListener('input', function() {
            if (clientInput.dataset.generalClient !== 'true') {
                generalClientBtn?.classList.remove('highlighted');
            }
        });
    }

    // On form reset, always reset General Client mode
    const form = document.getElementById('invoiceForm');
    if (form) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (currencySelect) {
        currencySelect.addEventListener('change', async function() {
            ...
            await window.invoiceForm.updateExchangeRate(currency);
            
            // Update all calculations with new currency
            document.querySelectorAll('#itemsTable .item-row').forEach(row => {
                window.invoiceForm.updateItemRow(row);
            });
            window.invoiceForm.updateReviewTotals();
            
            // Add currency note if needed
            const notesField = document.getElementById('notes');
            if (notesField && currency !== 'MZN') {
                const currencyNote = `Fatura emitida em ${currency}, com conversão para MZN à taxa oficial do BM à data da operação.`;
                if (!notesField.value.includes(currencyNote)) {
